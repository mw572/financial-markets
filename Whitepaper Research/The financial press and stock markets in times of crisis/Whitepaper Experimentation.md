...
Please find link to previous analysis piece here LINK IT
...
In R: a script (“newsdatacollection.R") ###  LINK THIS ###  was written to extract the dates and corresponding headline from the page of search results, a function was a utilised to iterate through all the available pages. The data was this is compiled together into a data frame which is saved as a Rdata file   LINK THIS    ("WSJ-14-15-Backup.Rdata") which will be utilised in the next step on calculating the bad news index. A CSV file was also saved incase of needed to manipulate the raw data or review it at a later date..
...
The index output from the BNI creation script (with the improved bad word dictionary) was saved as a CSV file (“WSJ-2014-15-Market-index.csv”)    LINK THIS   , the corresponding counts were saved as (“WSJ-2014-15-Market-counts.csv”) LINK THIS although these are not used in the next piece of analysis.


**Importing to Google Sheets**

Having now imported the index CSV data into Google Sheets, the following manipulations are performed in this sheet (https://goo.gl/zhRmFu) LINK THIS , with corresponding tab number:
