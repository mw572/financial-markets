...
#load up our extended dictionary from 08 recession headline wording
bad_extended <- read.delim(file='badwordsextended.txt', header=FALSE, stringsAsFactors=FALSE)

#categorize words as very neg to very pos 
vn <- c(afinn_list$word[afinn_list$score==-5 | afinn_list$score==-4], bad_extended)
n <- c(afinn_list$word[afinn_list$score==-3 | afinn_list$score==-2 | afinn_list$score==-1])
p <- c(afinn_list$word[afinn_list$score==3 | afinn_list$score==2 | afinn_list$score==1])
vp <- c(afinn_list$word[afinn_list$score==5 | afinn_list$score==4])
...
#creating a function to calculate number of words in each polarity category within a sentence
sentimentScore <- function(sentences, vn, n, p, vp){
  
  final_scores <- matrix('', 0, 5) #creating output location
  
  scores <- laply(sentences, function(sentence, vn, n, p, vp){
    
    initial_sentence <- sentence #saving a copy of text
...
a <- 1 #helping us iterate through days correctly as they are spread across multiple rows
#iterate through this scoring function for each day 
for(i in 1:nrow(scrapedfile)){
  ...
  print(paste(signif((i/nrow(scrapedfile))*100,2),"%")) #progress meter
}
...
headlines <- as.data.frame() #precreating dataframe
...
counts = counts[-1,] #remove the first row from dataframe that was precreated

#save table of data as a backup
...
badnewsindex = badnewsindex[-1,] #removing first row from precreated dataframe

#save data as CSV so it can be imported into Google Sheets for analysis
